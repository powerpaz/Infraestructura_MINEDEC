...
async function loadInstituciones(){
  ...
  for(const row of data){
    const keys = Object.keys(row);
    const find = (target) => keys.find(k => k && k.toLowerCase() === target);
    // Usamos exclusivamente las columnas finales "longitud" y "latitud" (case-insensitive)
    let lon = toNum(row[find('longitud')]);
    let lat = toNum(row[find('latitud')]);
